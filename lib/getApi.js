...
const parametersFilterFun = (data,definitions)=>{
  ...
  if (_.isNil(data)) {
    ...
    data.forEach(item => {
      // console.log(item,'666',definitions);
      if (item.in == 'query') {
        ...
        if (_.has(item.schema,'$ref')) {
          ...
          // console.log(item.schema['$ref'],isLast,lastObj,schema,'schema');
          ...
          // });
          const parameters = Object.entries(data).reduce((obj, [key, value]) => {
      ...
      const apiPaths = Object.entries(data.paths).map(([key, value]) => {
        ...
        return {
          ...
          ...Object.entries(value).reduce((acc, [secondKey, secondValue]) => {
            ...
            // console.log(parametersKey,path,secondValue,"parametersKey");
            //去除特殊字符~!@#$^-&*()=|{}':;',\[].<>/?~！@#￥……&*（）——|{}【】'；：""'。，、？
...
const generateCode = (titleInfo, configs, rule, methodArr) => {
  const jsCode = command == 'api' ? axiosTemplate({ titleInfo, configs, rule, methodArr }) : mgopTemplate({ titleInfo, configs, rule, methodArr });
  fileSystem.writeFileSync(path.join(path.join(process.cwd(), rule.apiPath || 'src/server'), rule.fileName || `${rule.server.split('/').pop()}Api.js`), jsCode, {
    encoding: 'utf-8',
  });
  console.log(rule.fileName);
  rule.fileName != undefined ? success(`------${rule.fileName +'接口文件已生成-----'}`) : success(`------${rule.server.split('/').pop()+'Api.js接口文件已生成-----'}`);
};
