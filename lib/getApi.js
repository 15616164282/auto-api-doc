...
const generateCode = (titleInfo, configs, rule, methodArr) => {
  const jsCode = rule.mgopName == undefined ? axiosTemplate({ titleInfo, configs, rule, methodArr })  :  mgopTemplate({ titleInfo, configs, rule, methodArr });
  fileSystem.writeFileSync(path.join(path.join(process.cwd(), rule.apiPath || 'src/http'), rule.fileName || `${rule.server.split('/').pop()}Api.js`), jsCode, {
    encoding: 'utf-8',
  });
  success(`------${rule.fileName || rule.server.split('/').pop()}Api.js接口文件已生成-----`);
};
